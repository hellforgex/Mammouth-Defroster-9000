...
# Active nonce cache for HMAC challenge-response (1 attempt/nonce, TTL 30s)
_ACTIVE_NONCES = {}
NONCE_TTL_SECONDS = 30.0

def _generate_hmac_challenge() -> str:
    """Generate a single-use cryptographically secure nonce with 30s TTL."""
    now = _time.time()
    expired = [n for n, t in _ACTIVE_NONCES.items() if now - t > NONCE_TTL_SECONDS]
    for n in expired:
        _ACTIVE_NONCES.pop(n, None)
    nonce = _secrets.token_hex(16)
    _ACTIVE_NONCES[nonce] = now
    return nonce

def _compute_hmac_response(token: str, nonce: str) -> str:
    """Compute HMAC-SHA256 response for a given challenge nonce."""
    return _hmac.new(token.encode("utf-8"), nonce.encode("utf-8"), _hashlib.sha256).hexdigest()

def _verify_hmac_response(token: str, nonce: str, response_hmac: str) -> bool:
    """Verify single-use HMAC-SHA256 response against active nonce cache (Fail-closed)."""
    now = _time.time()
    created_time = _ACTIVE_NONCES.pop(nonce, None)
    if created_time is None:
        return False
    if now - created_time > NONCE_TTL_SECONDS:
        return False
    expected = _compute_hmac_response(token, nonce)
    return _secrets.compare_digest(expected, response_hmac)
...
class _RemoteExecutionBroadcastConnection(object):
    ...
    def broadcast_open_connection(self, remote_node_id):
        ...
        '''
        challenge_nonce = _generate_hmac_challenge()
        payload = {
            'command_ip': self._config.command_endpoint[0],
            'command_port': self._config.command_endpoint[1],
            'challenge_nonce': challenge_nonce,
        }
        self._broadcast_message(_RemoteExecutionMessage(_TYPE_OPEN_CONNECTION, self._node_id, remote_node_id, payload))
...
class _RemoteExecutionCommandConnection(object):
    ...
    def _try_accept(self, broadcast_connection):
        ...
        for _n in range(6):
            ...
            try:
                client_sock, client_addr = self._command_listen_socket.accept()
                # M-03: Security Gate - Enforce that the command connection strictly originates from loopback (127.0.0.1)
                if client_addr[0] not in ("127.0.0.1", "::1", "localhost"):
                    client_sock.close()
                    raise PermissionError(f"Security Shield: Remote Execution rejected unauthorized non-loopback connection from {client_addr}")
                self._command_channel_socket = client_sock
